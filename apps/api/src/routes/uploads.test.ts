...
describe('Uploads Routes', () => {
  ...
  describe('POST /', () => {
    it('should upload a valid JPEG file', async () => {
      ...
      expect(mockUploadsService.upload).toHaveBeenCalledWith(expect.any(File), undefined);
    });
    ...
    it('should return 400 when file is too large', async () => {
      ...
      formData.append(
        'file',
        new File(['a'.repeat(11_000_000)], 'large.jpg', { type: 'image/jpeg' }),
      );
    ...
    it('should return 400 for invalid MIME type', async () => {
      mockUploadsService.upload.mockRejectedValue(
        new Error(
          'Invalid MIME type: text/plain. Allowed types: image/jpeg, image/png, image/gif, image/webp',
        ),
      );
